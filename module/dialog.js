...
const TALISMAN_SIZES = [1, 2, 3, 4, 5, 6, 7, 8];
...
export class ClockAddDialog extends fapi.HandlebarsApplicationMixin(fapi.Application) {
    ...
    get title() {
        const key = (this.entry ? "EditTitle" : "Title") + (this.type === "points" ? "Points" : (this.type === "talisman" ? "Talisman" : ""));
    ...
    static #onUpdateObject(event, _form, formData) {
        ...
        data.type ??= this.type;
        data.max = this.type === "points" ? 99 : (this.type === "talisman" ? Math.min(data.max, 8) : Math.max(data.max, 1));
        if (this.entry) {
