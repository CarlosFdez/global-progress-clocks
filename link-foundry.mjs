...
const MODULE_NAME = "global-progress-clocks";
...
const symlinkPath = path.resolve(dataPath, "modules", MODULE_NAME);
...
if (symlinkStats) {
    ...
    const proceed = (
        await prompts({
            ...
            name: "value",
            initial: false,
            message: `A "${MODULE_NAME}" ${atPath} already exists in the "systems" subfolder. Replace with new symlink?`,
        })
    ).value;
    if (!proceed) {
